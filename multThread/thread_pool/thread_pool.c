...
typedef struct {
    ...
    pthread_mutex_t thread_counter;     /*记录忙状态线程数的锁--busy_thr_num */
...
*/
    ...
    int shutdown;               /*标志位，线程池的使用状态，true or false */
}threadpool_t;
...
threadpool_t* threadpool_create(int min_thr_num, int max_thr_num, int queue_max_size)
{
    ...
    do{
        ...
        pthread_create(&(pool->adjust_tid),NULL,adjust_thread,(void *)pool);        /* 启动管理者进程 */

        return pool;

    } while(0);

    threadpool_free(pool);  /*前面代码失败是，释放pool存储空间*/
...
void *threadpool_thread(void *threadpool)
{
    ...
    while(true){
        ...
        /*
刚创建出线程，等待任务队列里有任务，否则阻塞等待任务队列里有任务后再唤醒接收任�
� */
        pthread_mutex_lock(&(pool->lock));
        ...
        while(pool->queue_size ==0 && (!pool->shutdown)){
            ...
            /* 清楚指定数码的空闲进程 ，如果要结束的进程格式大于0，结束进程 */
            if(pool->wait_exit_thr_num > 0){
                pool->wait_exit_thr_num--;

                /* 如果线程池里线程线程个数大于最小值可以结束当前进程 */
        ...
        (*(task.function))(task.arg);   /*执行回掉函数任务*/
...
void *adjust_thread(void *threadpool)
{
    ...
    while(!pool->shutdown){
        ...
        /*创建线程算法：任务数大于最小线程个数，且存活的线程数少于最大线程个数数时 如 30>=10 && 40<100 */
        ...
        /*销毁多余的空闲线程 算法：忙线程x2 小于存活线程数 且存活的线程数大于最小线程数 时*/
...
int threadpoll_all_threadnum(threadpool_t *pool)
{
    int all_threadnum = -1;
    pthread_mutex_lock(&(pool->lock));
...
int main(void)
{
    ...
    for(i=0;i<40;i++){
        num[i]=i;
        printf("add task %d\n",i);
        threadpool_add(thp,process,(void*)&num[i]);     /*向进程池中添加任务 */
    }
    sleep(10);                          /*等待线程完成任务 */
